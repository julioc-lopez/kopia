...
func (e *Environment) setup(tb testing.TB, version content.FormatVersion, opts ...Options) *Environment {
	...
	return e.initializeRepo(tb, version, opts...).openRepo(tb, opts...)
}

func (e *Environment) initializeRepo(tb testing.TB, version content.FormatVersion, opts ...Options) *Environment {
	tb.Helper()
	...
	if e.configDir == "" {
		...
		if err := os.MkdirAll(e.configDir, 0o755); err != nil {
			tb.Fatal("creating config dir:", e.configDir, ":", err)
		}
	}
	...
	st = repotesting.NewReconnectableStorage(tb, st)
	e.st = st
...
func (e *Environment) openRepo(tb testing.TB, opts ...Options) *Environment {
	tb.Helper()
...
func repoOptions(repoOpts []func(*repo.Options)) *repo.Options {
	opts := &repo.Options{}

	for _, mod := range repoOpts {
		if mod != nil {
			mod(opts)
		}
	}
...
func NewEnvironment(tb testing.TB, version content.FormatVersion, configDir string, opts ...Options) (context.Context, *Environment) {
	tb.Helper()
...
func OpenEnvironment(tb testing.TB, configDir string, opts ...Options) (context.Context, *Environment) {
	tb.Helper()
